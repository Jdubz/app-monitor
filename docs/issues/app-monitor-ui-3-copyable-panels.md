# APP-MONITOR-UI-3 — Fix Ctrl+C to Copy Selected Text Instead of Clearing Logs

## Issue Metadata

```yaml
Title: APP-MONITOR-UI-3 — Fix Ctrl+C to Copy Selected Text Instead of Clearing Logs
...
The app-monitor provides centralized monitoring for all development services, and this fix will improve user experience by providing standard copy-to-clipboard behavior that users expect.
...
```yaml
MODIFY:
  - app-monitor/frontend/src/components/LogsViewer.tsx - Fix Ctrl+C behavior
  - app-monitor/frontend/src/components/LogsToolbar.tsx - Update clear logs shortcut
  - app-monitor/frontend/src/hooks/useLogStream.ts - Add clipboard integration
  - app-monitor/frontend/src/components/LogLine.tsx - Improve text selection
  - app-monitor/frontend/src/utils/clipboard.ts - Clipboard utility functions

CREATE:
  - app-monitor/frontend/src/hooks/useClipboard.ts - Clipboard management hook
  - app-monitor/frontend/src/utils/textSelection.ts - Text selection utilities
```
...
```bash
# Start app-monitor with clipboard integration
npm run dev
