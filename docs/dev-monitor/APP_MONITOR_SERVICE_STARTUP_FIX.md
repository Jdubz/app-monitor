...
The app-monitor had several service startup and cleanup issues:
...
3. **No Cleanup on Exit** - App Monitor didn't stop services when it was closed
4. **Docker Orphans** - Docker containers kept running after app-monitor exit
...
- **Graceful cleanup on exit** - Stop all services when app-monitor closes
...
**File:** `app-monitor/backend/src/utils/portManager.ts` (NEW)
...
**File:** `app-monitor/backend/src/services/processManager.ts` (MODIFIED)
...
From `app-monitor/backend/src/config.ts`:
...
### On App Monitor Exit (SIGTERM/SIGINT)
...
- All services stopped when app-monitor exits
...
# 2. Try to start firebase-emulators via app-monitor
...
# 2. Try to start python-worker via app-monitor
...
# 2. Start all services via app-monitor UI
# 3. Stop app-monitor (Ctrl+C)
...
- `app-monitor/backend/src/utils/portManager.ts` (165 lines)
...
- `app-monitor/backend/src/services/processManager.ts`
...
- [x] Clean shutdown when app-monitor exits

---

## Conclusion

Successfully implemented comprehensive service lifecycle management that eliminates port conflicts, ensures clean startup, and provides graceful shutdown. The app-monitor can now reliably start and stop services without manual intervention or orphaned processes.
