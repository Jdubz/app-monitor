...
**File:** `/home/jdubz/Development/job-finder-app-manager/app-monitor/backend/src/config.ts`
...
| Dev Monitor Backend  | 5000                                                                                                   | app-monitor/backend  |
| Dev Monitor Frontend | 5174                                                                                                   | app-monitor/frontend |

**Note:** Dev Monitor Backend port 5000 may conflict with Firebase Hosting emulator (also 5000). This should be monitored or the app-monitor backend port should be changed if running simultaneously.
...
The app-monitor backend runs on port 5000, which conflicts with Firebase Hosting emulator (also port 5000).

**Options:**

1. Change app-monitor backend to different port (e.g., 5002)
2. Document that app-monitor should run independently
3. Disable Firebase Hosting emulator if not needed for development

**Recommendation:** Change app-monitor backend to port 5002 to avoid conflicts.
