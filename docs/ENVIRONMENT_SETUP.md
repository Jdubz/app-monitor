...
4. Add the following secret:
   - Name: `VITE_PASSWORD`
   - Value: Your production password (same as on server)

## How It Works

### CI/CD Pipeline

1. GitHub Actions builds the frontend using the `VITE_PASSWORD` secret
2. The password is embedded in the built JavaScript bundle
3. The deployment artifact is uploaded to GitHub

### Production Deployment

1. The deploy agent downloads the artifact
2. The deployment script extracts it to the production server
3. The script rebuilds the frontend, loading `VITE_PASSWORD` from `/opt/app-monitor/shared/config/.env.production`
...
- ✅ Production passwords are stored in GitHub Secrets (encrypted)
...
1. Check if the password is set in GitHub Secrets:
   ```bash
   # You can't view the secret value, but check the deployment logs
   gh run list --workflow=deploy-production.yml --limit 1
   ```

2. Check if the password is set on the production server:
   ```bash
   ssh your-server
...
3. Rebuild the frontend if the password was just added:
   ```bash
   cd /opt/app-monitor/current/frontend
   export VITE_PASSWORD=your-password
   npm run build
   ```
