...
| APP_MONITOR_PRODUCTION_SUPPORT_PLAN.md | Partial | Systemd unit + cleanup workflow (`scripts/systemd/app-monitor-backend@.service`, `scripts/production/cleanup-processes.sh`) now control prod processes. | Define/enforce `APP_MONITOR_ROOT/DEPLOY_ROOT/ARTIFACT_ROOT` env contracts and multi-root registry entries. |
...
| BETTER_PROCESS_MANAGEMENT.md | ✅ Complete | `backend/src/utils/singleInstance.ts` blocks duplicate processes; `scripts/production/cleanup-processes.sh` handles blue/green cleanup. | None. |
...
| DATABASE_CONSOLIDATION_PLAN.md | ✅ Complete | All services now use `backend/data/app-monitor.db` (configured via `config.databasePath`). Legacy `dev-bots.db` and `tasks/queue.db` exist but are unused. | Clean up legacy database files and update plan status. |
| DATABASE_REORGANIZATION.md | ✅ Complete | Consolidated to single `app-monitor.db`. Legacy files in backups/. | Archive this plan (superseded by consolidation). |
| DEV_BOT_INTERACTIVE_SESSION_TAB.md | ✅ Complete | Frontend tab (`frontend/src/components/dev-bots/interactive/InteractiveSessionTab.tsx`) + backend gateway (`backend/src/services/interactiveSessionGateway.ts`) are live. | Future UX iterations (RBAC/chat) only. |
...
| INTELLIGENT_AGENT_SELECTION_STRATEGY.md | ✅ Complete | `backend/src/services/agentSelector.ts` + `taskClassifier.ts` route tasks to the right agent. | Monitor selection metrics, but no remaining scope here. |
| PR_CREATION_AUTOMATION_RESTORE_PLAN.md | Not started | Containers still resolve `$HOME` incorrectly and no fallback token is wired (`backend/src/services/ephemeralWorker.service.ts`). | Deliver HOME override, token distribution, health checks. |
...
| STUCK_PRODUCTION_PRS_AUTOMATION_PLAN.md | Partial | Systemd + cleanup scripts deployed, preventing duplicate processes. Metrics (`webhook_events_processed`) still unimplemented. | Add webhook heartbeat metric/alerts + dashboard. |
| WEBSOCKET_RESILIENCE_STRATEGY.md | Partial | Client reconnection/health lives in `frontend/src/services/socketService.ts`; backend `connectionManager.ts` tracks clients but lacks cross-instance state sync. | Implement state handoff/queue + shared session storage for blue/green. |
