...
export class CostMonitor {
  ...
  public recordUsage(tokens: number, cost: number): boolean {
    ...
    if (tokens > this.config.tokens.maxTokensPerRequest) {
      console.error(`Request blocked: ${tokens} tokens exceeds max ${this.config.tokens.maxTokensPerRequest}`);
      return false;
    }

    if (cost > this.config.spending.perTaskLimit) {
      console.error(`Request blocked: $${cost} exceeds per-task limit $${this.config.spending.perTaskLimit}`);
      return false;
    }
    ...
    this.metrics.averageTokensPerRequest = this.metrics.totalTokens / this.metrics.requestCount;
  ...
  private checkSpendingLimits(): boolean {
    ...
    if (this.metrics.dailyCost >= dailyLimit * emergencyThreshold) {
      console.warn(`⚠️ Daily spending approaching limit: $${this.metrics.dailyCost}/${dailyLimit}`);
    }

    if (this.metrics.dailyCost >= dailyLimit) {
      console.error(`🚨 Daily spending limit exceeded: $${this.metrics.dailyCost}/${dailyLimit}`);
    ...
    if (this.metrics.monthlyCost >= monthlyLimit * emergencyThreshold) {
      console.warn(`⚠️ Monthly spending approaching limit: $${this.metrics.monthlyCost}/${monthlyLimit}`);
    }

    if (this.metrics.monthlyCost >= monthlyLimit) {
      console.error(`🚨 Monthly spending limit exceeded: $${this.metrics.monthlyCost}/${monthlyLimit}`);
      this.triggerEmergencyStop();
      return false;
    }
  ...
  private sendEmergencyNotification(): void {
    ...
    const notificationFile = path.join(__dirname, '../logs/emergency-notifications.log');
  ...
  private generateUsageReport(): void {
    const report = {
      ...
      utilization: {
        ...
        monthly: (this.metrics.monthlyCost / this.config.spending.monthlyLimit) * 100
      }
    };
  ...
  public isRequestAllowed(estimatedTokens: number): boolean {
    ...
    // Check daily limits
    if (this.metrics.dailyCost + estimatedCost > this.config.spending.dailyLimit) {
      return false;
    }

    // Check monthly limits
    if (this.metrics.monthlyCost + estimatedCost > this.config.spending.monthlyLimit) {
      return false;
    }
