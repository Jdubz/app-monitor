...
export class DebugLoopDetector {
  ...
  private checkProcessForLoops(monitor: ProcessMonitor): void {
    ...
    if (monitor.iterations > this.config.detection.maxIterations) {
      this.detectLoop(monitor, 'infinite_loop', `Process ${monitor.pid} exceeded max iterations (${monitor.iterations})`);
    }

    // Check for timeout loops
    if (duration > this.config.detection.maxDuration) {
      this.detectLoop(monitor, 'timeout_loop', `Process ${monitor.pid} exceeded max duration (${duration}ms)`);
    }

    // Check for stuck processes
    if (timeSinceActivity > 60000) { // 1 minute without activity
      this.detectLoop(monitor, 'debug_loop', `Process ${monitor.pid} appears stuck (${timeSinceActivity}ms since last activity)`);
    }

    // Check for pattern-based loops
    const loopPattern = this.detectPatternLoop(monitor.patterns);
    if (loopPattern) {
      this.detectLoop(monitor, 'error_loop', `Process ${monitor.pid} detected pattern loop: ${loopPattern}`);
    }
  }
  ...
  private detectPatternLoop(patterns: string[]): string | null {
    ...
    for (const [pattern, count] of Object.entries(patternCounts)) {
      if (count >= this.config.detection.patternThreshold * recentPatterns.length) {
        return pattern;
      }
    }
   ...
   */
  private detectLoop(monitor: ProcessMonitor, type: LoopDetection['type'], description: string): void {
    ...
    console.warn(`Type: ${type}, Iterations: ${monitor.iterations}, Duration: ${detection.duration}ms`);
   ...
   * Attempt to resolve the loop
   */
  private attemptResolution(monitor: ProcessMonitor, detection: LoopDetection): string | null {
   ...
   */
  private escalateLoop(monitor: ProcessMonitor, detection: LoopDetection): void {
  ...
  public getDetectionStats(): {
    ...
    const resolvedLoops = detections.filter(d => d.resolution && !d.resolution.includes('killed')).length;
    const killedProcesses = detections.filter(d => d.resolution && d.resolution.includes('killed')).length;
  ...
  public clearOldDetections(maxAge: number = 24 * 60 * 60 * 1000): void {
    ...
    const oldDetections = Array.from(this.detections.entries())
      .filter(([, detection]) => new Date(detection.firstDetected).getTime() < cutoff);
