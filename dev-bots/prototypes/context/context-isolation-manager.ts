...
export class ContextIsolationManager {
  ...
  public async createIsolatedContext(
    ...
    if (this.activeSessions.size >= this.config.sessions.maxConcurrentSessions) {
...
Execute the task now.`;
      ...
      const outputFile = path.join(__dirname, '../results', `task-${taskId}-${Date.now()}.json`);
      const logFile = path.join(__dirname, '../logs', `task-${taskId}-${new Date().toISOString().split('T')[0]}.log`);
  ...
  private async calculateContextSize(contextDir: string): Promise<number> {
    ...
    const files = await fs.promises.readdir(contextDir, { withFileTypes: true });
  ...
  public async cleanupContext(taskId: string): Promise<void> {
    ...
    try {
      ...
      await fs.promises.rm(taskContext.contextDir, { recursive: true, force: true });
  ...
  public getContextStats(): {
    ...
    const averageContextSize = contexts.length > 0 ? totalSize / contexts.length : 0;
