...
class TaskAdder {
  ...
  async addTask() {
    ...
    try {
      ...
      console.log(
        `Assigned Agent: ${AGENT_PERSONALITIES[taskData.assignedAgent] || "Auto-assigned"}`,
      );
    } catch (error) {
  ...
  async selectTaskType() {
    ...
    const typeKey = Object.keys(TASK_TYPES).find(
  ...
  async selectAgent() {
    ...
    console.log(
      `  ${agents.length + 1}. auto - Auto-assign based on task type`,
    );
    ...
    // Try to match by name
    const agentKey = Object.keys(AGENT_PERSONALITIES).find(
      (key) => key.toLowerCase() === choice.toLowerCase(),
    );
  ...
  async selectReviewType() {
    ...
    const typeKey = reviewTypes.find(
      (type) => type.toLowerCase() === choice.toLowerCase(),
    );
  ...
  async selectTestScope() {
    ...
    const scopeKey = testScopes.find(
      (scope) => scope.toLowerCase() === choice.toLowerCase(),
    );
