...
class ContextIsolation {
  ...
  getCleanContext(previousTaskId) {
    if (this.contaminatedContexts.has(previousTaskId)) {
      return (
        this.cleanContexts.get(previousTaskId) || this.getBaselineContext()
      );
    }
...
class SnowballPrevention {
  ...
  triggerChainBreaker(taskId, chain) {
    console.warn(
      `[CHAIN_BREAKER] Detected violation chain of ${chain.length} tasks`,
    );
...
app.post("/api/emergency-recovery", (req, res) => {
  ...
  const recoveryTask = snowballPrevention.createEmergencyRecoveryTask([
    { taskId },
  ]);
