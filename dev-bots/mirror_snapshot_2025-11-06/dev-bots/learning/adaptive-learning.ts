...
export class AdaptiveLearning {
  ...
  private isPatternRelevant(
    ...
    switch (pattern.type) {
      ...
      case "time":
        return (
          this.categorizeTimePattern(feedback.timeTaken) === pattern.pattern
        );
      default:
        return false;
    }
  ...
  private generateRecommendations(): string[] {
    ...
    const errorPatterns = Array.from(this.patterns.values()).filter(
      (p) => p.type === "error" && p.frequency > 3,
    );

    for (const pattern of errorPatterns) {
      if (pattern.prevention) {
        recommendations.push(
          `Prevent ${pattern.pattern}: ${pattern.prevention}`,
        );
      }
    }
    ...
    const successPatterns = Array.from(this.patterns.values()).filter(
      (p) => p.type === "success" && p.frequency > 2,
    );

    for (const pattern of successPatterns) {
      if (pattern.optimization) {
        recommendations.push(
          `Optimize ${pattern.pattern}: ${pattern.optimization}`,
        );
      }
    }
  ...
  public predictTaskSuccess(taskDescription: string): {
    ...
    for (const [, pattern] of this.patterns) {
      if (pattern.type === "error" && pattern.confidence > 0.5) {
        if (
          taskDescription.toLowerCase().includes(pattern.pattern.toLowerCase())
        ) {
    ...
    const timePatterns = Array.from(this.patterns.values()).filter(
      (p) => p.type === "time",
    );

    if (timePatterns.length > 0) {
      const avgTime =
        timePatterns.reduce((sum, p) => sum + p.frequency, 0) /
        timePatterns.length;
