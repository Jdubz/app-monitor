...
import {
  TestSocketIOServer,
  TestClientSocket,
} from "../helpers/fake-socket-server.js";
...
describe("Socket.IO event broadcasts (in-process)", () => {
  ...
  it("delivers process lifecycle events to connected clients", () => {
    ...
    client.clientOn("process:started", (data) =>
      events.push(`started:${data.processId}`),
    );
    client.clientOn("process:log", (data) =>
      events.push(`log:${data.processId}`),
    );
    client.clientOn("process:stopped", (data) =>
      events.push(`stopped:${data.processId}`),
    );
