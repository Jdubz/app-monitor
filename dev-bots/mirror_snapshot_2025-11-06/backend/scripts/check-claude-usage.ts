...
async function main() {
  ...
  try {
      ...
      summary.totalInputTokens > 0
        ? (
            (summary.totalCacheReadTokens /
              (summary.totalInputTokens + summary.totalCacheReadTokens)) *
            100
          ).toFixed(1)
        : "0.0";

    console.log(
      `Average Input Tokens/Request: ${avgInputPerRequest.toLocaleString()}`,
    );
    console.log(
      `Average Output Tokens/Request: ${avgOutputPerRequest.toLocaleString()}`,
    );
    console.log(`Cache Hit Rate: ${cacheHitRate}%`);
    console.log(
      `Cache Savings: ${summary.totalCacheReadTokens.toLocaleString()} tokens (free)`,
    );
    ...
    if (summary.estimatedCost > 0) {
      ...
      const daysInSummary = args.includes("--today")
        ? 1
        : args.includes("--month")
          ? 30
          : 7;
