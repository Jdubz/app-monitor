...
router.get("/summary/:provider", (req: Request, res: Response) => {
  try {
    ...
    logger.error(
      `Error getting token usage summary for ${req.params.provider}:`,
      error,
    );
...
router.get("/budget/:provider", (req: Request, res: Response) => {
  try {
    ...
    if (!budget) {
      return res
        .status(404)
        .json({ error: `No budget configured for provider: ${provider}` });
    }
...
router.put("/budget", (req: Request, res: Response) => {
  try {
    ...
    if (typeof budget.dailyLimit !== "number" || budget.dailyLimit <= 0) {
      return res
        .status(400)
        .json({ error: "Daily limit must be a positive number" });
    }
    if (
      typeof budget.costPerMillionInput !== "number" ||
      budget.costPerMillionInput < 0
    ) {
      return res
        .status(400)
        .json({
          error: "Cost per million input must be a non-negative number",
        });
    }
    if (
      typeof budget.costPerMillionOutput !== "number" ||
      budget.costPerMillionOutput < 0
    ) {
      return res
        .status(400)
        .json({
          error: "Cost per million output must be a non-negative number",
        });
    }
    if (
      typeof budget.warningThreshold !== "number" ||
      budget.warningThreshold < 0 ||
      budget.warningThreshold > 100
    ) {
      return res
        .status(400)
        .json({ error: "Warning threshold must be between 0 and 100" });
    }
...
router.get("/remaining/:provider", (req: Request, res: Response) => {
  try {
    ...
    logger.error(
      `Error getting remaining tokens for ${req.params.provider}:`,
      error,
    );
