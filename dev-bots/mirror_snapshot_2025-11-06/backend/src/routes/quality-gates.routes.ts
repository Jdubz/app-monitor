...
router.get("/config", (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get quality gate configurations" });
  }
});
...
router.put("/config/:gate", (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      typeof updates.required !== "boolean"
    ) {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "weight must be a number between 1 and 10" });
    }
    if (
      updates.timeout !== undefined &&
      (typeof updates.timeout !== "number" || updates.timeout <= 0)
    ) {
      return res
        .status(400)
        .json({ error: "timeout must be a positive number" });
    }
...
router.post("/validate", async (req: Request, res: Response) => {
  try {
    ...
    const result = await qualityGates.validateTask(
      taskId,
      workspacePath,
      project,
    );
