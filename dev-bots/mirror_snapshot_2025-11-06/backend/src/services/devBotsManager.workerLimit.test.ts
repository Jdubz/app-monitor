...
describe("DevBotsManager Worker Limit Enforcement", () => {
  ...
  beforeEach(() => {
    ...
    devBotsManager["initializeWorkerLogFile"] = vi
      .fn()
  ...
  describe("Worker Limit Enforcement", () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            "Assigning task task-1 to available worker",
          ),
        }),
      );
    });
    ...
    it.skip("should allow second task to be assigned to worker-b (integration test - fix in Phase 2)", async () => {
      ...
      expect(logger.info).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            "Assigning task task-2 to available worker",
          ),
        }),
      );
    });
    ...
    it("should handle worker cleanup errors gracefully", async () => {
      ...
      await expect(
        devBotsManager["destroyEphemeralWorker"]("bot-a-test-123"),
  ...
  describe("Worker Type Assignment Logic", () => {
    it("should correctly identify active worker-a", () => {
      ...
      const activeWorkers = Array.from(
        devBotsManager["ephemeralWorkers"].values(),
      ).filter((worker) => worker.status !== "destroyed");

      const hasBotA = activeWorkers.some((worker) =>
        worker.id.includes("bot-a"),
      );
    ...
    it("should correctly identify active worker-b", () => {
      ...
      const hasBotB = activeWorkers.some((worker) =>
        worker.id.includes("bot-b"),
      );
