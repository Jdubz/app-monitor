...
import { isPortInUse, stopDockerContainer } from "../utils/portManager.js";
...
export class ProcessManager extends EventEmitter {
  ...
  async startService(serviceName: string): Promise<ProcessInfo> {
    ...
    try {
      ...
      if (config.requirePorts && config.ports && config.ports.length > 0) {
        const { available, busyPorts } = await checkPortsAvailable(
          config.ports,
        );
      ...
      if (
        ...
        if (anyPortInUse) {
          ...
          logger.info({
            category: "api",
            action:
              "firebase_emulators_detected_running_on_ports_not_managed_by_dev_monitor",
    ...
    if (
      ...
      if (
        ...
        logger.warn({
          category: "process",
          action:
            "firebase_emulators_marked_as_status_status_but_ports_not_in_use_updating_to_stopped",
        ...
        logger.info({
          category: "api",
          action:
            "firebase_emulators_running_on_ports_updating_status_to_running",
  ...
  public getLogWatcher(): import("./logWatcher.js").LogWatcher | undefined {
    ...
    return (
      this as ProcessManager & {
        logWatcher?: import("./logWatcher.js").LogWatcher;
      }
    ).logWatcher;
  }
   ...
   */
  private async getDockerWorkerStatus(
   ...
   */
  private async waitForProcessStart(
    serviceName: string,
    timeout: number,
  ): Promise<void> {
  ...
  private async cleanupAll(): Promise<void> {
    ...
    const usingDocker = Object.values(services).some(
      (service) => service.command === "docker",
    );
