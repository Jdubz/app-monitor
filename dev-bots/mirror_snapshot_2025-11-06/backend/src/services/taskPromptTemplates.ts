...
import {
  getGuidelinesForTaskType,
  formatGuidelinesAsMarkdown,
} from "./taskTypeGuidelines.js";
...
export class TaskPromptTemplateManager {
  ...
  private variableProcessors: Map<string, (context: TaskContext) => string> =
    new Map();
...
- [ ] Review your changes on GitHub/GitLab
...
`;
  ...
  private initializeVariableProcessors(): void {
    ...
    this.variableProcessors.set(
      "task.title",
      (context) => context.task.title || "Untitled Task",
    );
    this.variableProcessors.set(
      "task.description",
      (context) =>
        context.task.description || "No detailed description provided",
    );
    this.variableProcessors.set(
      "task.documentation",
      (context) =>
        context.task.documentation ||
        "See project README and architecture docs",
    );
    ...
    this.variableProcessors.set(
      "task.notes",
      (context) => context.task.notes || "No additional notes",
    );
    ...
    this.variableProcessors.set("task.acceptanceCriteriaList", (context) =>
      ...
      Array.isArray(context.task.acceptanceCriteria) &&
    ...
    this.variableProcessors.set("task.architectureReferences", (context) => {
      ...
      const customRefs =
        context.task.architectureReferences &&
        context.task.architectureReferences.length > 0
    ...
    this.variableProcessors.set("task.contextBoundaries", (context) => {
      if (!context.task.contextBoundaries)
        return "- No specific boundaries defined";
      const {
        mustNotChange = [],
        mustNotAffect = [],
        integrationPoints = [],
      } = context.task.contextBoundaries;
    ...
    this.variableProcessors.set("task.testingRequirements", (context) =>
      context.task.testingRequirements &&
      context.task.testingRequirements.length > 0
        ? context.task.testingRequirements
            .map((t: string) => `- [ ] ${t}`)
            .join("\n")
    ...
    this.variableProcessors.set("task.documentationRequirements", (context) =>
      context.task.documentationRequirements &&
      context.task.documentationRequirements.length > 0
        ? context.task.documentationRequirements
            .map((d: string) => `- [ ] ${d}`)
            .join("\n")
...
- If significantly under/over estimate, note why for future planning`;
    ...
    this.variableProcessors.set(
      "task.parentInitiative",
      (context) =>
        context.task.parentInitiative ||
        "No parent initiative specified - this is an independent task",
    );

    // 6. Related tasks - coordination context
    this.variableProcessors.set("task.relatedTasks", (context) => {
      if (
        !context.task.relatedTasks ||
        context.task.relatedTasks.length === 0
      ) {
        return "- None - this task is independent and has no direct dependencies or dependents";
      }

      return context.task.relatedTasks
        .map((t: string) => {
          ...
          if (
            t.toLowerCase().includes("similar") ||
    ...
    this.variableProcessors.set("task.alternatives", (context) => {
      if (
        ...
        context.task.alternatives.length === 0
      ) {
      ...
      return context.task.alternatives
        .map((a: string) => `- ❌ ${a}`)
        .join("\n");
    });
