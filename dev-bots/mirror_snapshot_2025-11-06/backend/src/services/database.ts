...
export class DevBotsDatabase {
  ...
  recordTaskExecution(data: TaskExecution): void {
    ...
    `,
      ...
      .run(
        ...
        data.tokenInput,
        data.tokenOutput,
        data.complexityEstimated,
        data.complexityActual,
        data.qualityScores.completion,
        data.qualityScores.codeQuality,
        data.qualityScores.testCoverage,
        data.qualityScores.process,
        data.qualityScores.efficiency,
  ...
  getBatchApproval(id: number): BatchApproval | undefined {
    const result = this.db
      .prepare("SELECT * FROM batch_approvals WHERE id = ?")
      .get(id);
  ...
  updateBatchExecution(id: number, executedCount: number): void {
    this.db
      .prepare("UPDATE batch_approvals SET executed_count = ? WHERE id = ?")
  ...
  getFailurePatterns(taskId: string): FailurePattern[] {
    const results = this.db
      .prepare("SELECT * FROM failure_patterns WHERE task_id = ?")
      .all(taskId);
