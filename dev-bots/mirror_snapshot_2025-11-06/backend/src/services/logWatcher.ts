...
export class LogWatcher {
  ...
  private discoverLogFiles(): Array<{ filepath: string; service: string }> {
    ...
    try {
      ...
      const devMonitorLogFile = path.join(
        this.logDir,
      ...
      logger.info({
        ...
        message:
          "Only dev-monitor-backend logs will be streamed. External service logs use file reads only.",
        details: {
          streamingService: "dev-monitor-backend",
          fileBasedServices: ["frontend", "worker", "firebase-emulators"],
        },
      });
    } catch (error) {
      logger.error({
  ...
  private validateLogFormat(filepath: string): LogFormatValidation {
    try {
      ...
      if (jsonCount > plainTextCount) {
        return {
          format: "json",
          confidence: jsonCount === lines.length ? "high" : "low",
        };
      } else if (plainTextCount > jsonCount) {
        return {
          format: "plain-text",
          confidence: plainTextCount === lines.length ? "high" : "low",
        };
      } else {
    ...
    if (formatValidation.format === "plain-text") {
      logger.warn({
        ...
        details: {
          ...
          suggestion:
            "Move plain text logs to /logs/plain/ directory for automatic conversion",
        },
      });
    } else if (
      formatValidation.format === "mixed" ||
      formatValidation.format === "unknown"
    ) {
      logger.warn({
        category: "system",
        ...
        details: {
          ...
          confidence: formatValidation.confidence,
          suggestion:
            "Ensure log files contain only JSON or move to /logs/plain/ for plain text",
        },
      });
    }
  ...
  private processLogLine(
    ...
    if (isPlainTextLog) {
      ...
      const entry: StructuredLogEntry = this.convertPlainTextToStructured(
        line,
        service,
      );
   ...
   */
  private convertPlainTextToStructured(
    line: string,
    service: string,
  ): StructuredLogEntry {
    ...
    let category = "system";
    if (
      lowerLine.includes("emulator") ||
      lowerLine.includes("firestore") ||
        ...
        try {
          ...
          for (const line of recentLines) {
            try {
              ...
              // Convert plain text to structured format
              const structuredEntry = this.convertPlainTextToStructured(
                line,
                service,
              );
