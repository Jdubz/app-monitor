...
export class TokenTrackingService extends EventEmitter {
  ...
  private initializeDefaultBudgets(): void {
    ...
    this.setBudget({
      ...
      costPerMillionInput: 3.0, // $3 per 1M input tokens (Sonnet)
      costPerMillionOutput: 15.0, // $15 per 1M output tokens
    ...
    this.setBudget({
      ...
      costPerMillionInput: 10.0, // $10 per 1M tokens (estimated)
      costPerMillionOutput: 30.0, // $30 per 1M tokens (estimated)
      warningThreshold: 80,
    });
    ...
    this.setBudget({
      provider: "openai",
      dailyLimit: 500000, // 500K tokens per day
      costPerMillionInput: 10.0, // $10 per 1M input tokens
      costPerMillionOutput: 30.0, // $30 per 1M output tokens
      warningThreshold: 80,
    });
  ...
  private calculateCost(
    ...
    if (!budget) {
      logger.warn(
        `TokenTrackingService: No budget configured for ${provider}, cannot calculate cost`,
      );
      return 0;
    }
  ...
  resetDailyTracking(): void {
    ...
    logger.info(
      "TokenTrackingService: Daily tracking reset (database queries by date)",
    );
 ...
 */
export function getTokenTrackingService(
