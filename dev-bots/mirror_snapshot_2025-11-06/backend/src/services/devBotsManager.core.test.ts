...
vi.mock("./taskCreationGuidelines.js", async () => {
  return {
    TaskCreationGuidelinesManager: class MockTaskCreationGuidelinesManager {
      ...
      getGuidelinesForType = vi
        .fn()
        .mockReturnValue({ type: "feature", guidelines: [] });
      constructor() {}
    },
  };
});
...
describe.skip("DevBotsManager Core Functionality", () => {
  ...
  describe("Worker Management", () => {
    it("should create and manage worker lifecycle", async () => {
      ...
      expect(devBotsManager["workers"].get(workerId)?.personality).toEqual(
        agentPersonality,
      );
  ...
  describe("Task Retry System", () => {
    ...
    it("should respect max retries limit", async () => {
      ...
      const status = await devBotsManager.getSystemStatus();
      const completedTask = status.tasks.completed.find(
        (t) => t.id === "task-1",
      );
  ...
  describe("Task Cancellation", () => {
    it("should cancel running task", async () => {
      ...
      await devBotsManager["failEphemeralTask"](
        { id: "worker-1" } as any,
        "cancelled",
      );
  ...
  describe("Error Handling", () => {
    ...
    it("should handle Docker container failures", async () => {
      ...
      mockDockerManager.createContainer.mockRejectedValue(
