...
vi.mock("./logWatcher.js", async () => {
  const actual =
    await vi.importActual<typeof import("./logWatcher.js")>("./logWatcher.js");
  return {
    ...actual,
...
describe("LogStreamer", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should log initialization", () => {
      ...
      expect(logger.info).toHaveBeenCalledWith({
        ...
        message:
          "LogStreamer initialized - using file-based log ingestion only",
      });
    });
  });
  ...
  describe("Service Status Management", () => {
    ...
    it("should handle service status request", () => {
      ...
      const statusHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "get_service_status",
      )[1];
      ...
      // Then: Status is sent
      expect(mockProcessManager.getServiceStatus).toHaveBeenCalledWith(
        "backend",
      );
      expect(mockSocket.emit).toHaveBeenCalledWith("service_status", {
        status: "running",
      });
    });
    ...
    it("should handle all statuses request", () => {
      ...
      const allStatusHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "get_all_statuses",
      )[1];
  ...
  describe("Cloud Logs Management", () => {
    ...
    it("should handle cloud logs subscription", async () => {
      ...
      const cloudSubscribeHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "subscribe_cloud_logs",
      )[1];

      // When: Subscribing to cloud logs
      await cloudSubscribeHandler({
        environment: "staging",
        service: "backend",
        severity: "ERROR",
      });
    ...
    it("should handle cloud logs subscription error", async () => {
      ...
      mockCloudLogging.getLogs.mockRejectedValue(
        new Error("Cloud logging unavailable"),
      );
    ...
    it("should handle cloud logs unsubscription", () => {
      ...
      const cloudUnsubscribeHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "unsubscribe_cloud_logs",
      )[1];
    ...
    it("should handle cloud logs refresh", async () => {
      ...
      const refreshHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "refresh_cloud_logs",
      )[1];
    ...
    it("should handle environments request", () => {
      ...
      const environmentsHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "get_environments",
      )[1];
    ...
    it("should handle cloud logging status check", () => {
      ...
      const statusHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "check_cloud_logging_status",
      )[1];
  ...
  describe("File Logs Management", () => {
    ...
    it("should handle file logs subscription", () => {
      ...
      const fileSubscribeHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "subscribe_file_logs",
      )[1];
    ...
    it("should handle file logs unsubscription", () => {
      ...
      const fileUnsubscribeHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "unsubscribe_file_logs",
      )[1];
    ...
    it("should handle file log history request", () => {
      ...
      const historyHandler = mockSocket.on.mock.calls.find(
        (call) => call[0] === "get_file_log_history",
      )[1];
