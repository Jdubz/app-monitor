...
export class WorkspaceSyncManager extends EventEmitter {
  ...
  constructor(options: SyncOptions = {}) {
    ...
    this.baseDir =
      options.baseDir || path.resolve(path.join(process.cwd(), "../../"));
   ...
   */
  private async syncMainRepositories(
    options: SyncOptions,
    result: SyncResult,
  ): Promise<void> {
    logger.info({
   ...
   */
  private async syncMainRepository(
    repoName: string,
    repoPath: string,
    options: SyncOptions,
  ): Promise<void> {
   ...
   */
  private async syncWorkerWorkspaces(
    workerName: string,
    options: SyncOptions,
    result: SyncResult,
  ): Promise<void> {
   ...
   */
  private getGitStatus(repoPath: string): {
  ...
  private async handleUncommittedChanges(
    ...
    if (this.conflictStrategy === "stash") {
      ...
      this.executeGitCommand(
        repoPath,
        `git commit -m "Auto-commit before sync: ${new Date().toISOString()}"`,
        options,
      );
      logger.info({
        category: "process",
  ...
  private async handleMergeConflict(
    ...
    if (this.conflictStrategy === "auto-merge") {
      ...
      this.executeGitCommand(
        repoPath,
        'git stash push -m "Stashed due to merge conflict"',
        options,
      );
      // Then try to merge staging
      this.executeGitCommand(
        repoPath,
        'git merge origin/staging --no-ff -m "Sync with staging (after stash)"',
        options,
      );
      logger.info({
        category: "process",
  ...
  private isSimpleConflict(content: string): boolean {
    ...
    for (const match of matches) {
      ...
      const headNormalized = headPart
        .replace(/\s+/g, " ")
        .replace(/\/\/.*$/gm, "")
        .trim();
      const stagingNormalized = stagingPart
        .replace(/\s+/g, " ")
        .replace(/\/\/.*$/gm, "")
        .trim();
  ...
  private resolveSimpleConflict(content: string): string {
    ...
    return content.replace(conflictRegex, (match, headPart, stagingPart) => {
      ...
      return headTrimmed.length >= stagingTrimmed.length
        ? headTrimmed
        : stagingTrimmed;
    });
  }
