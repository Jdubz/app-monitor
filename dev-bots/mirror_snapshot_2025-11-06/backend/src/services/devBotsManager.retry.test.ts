...
vi.mock("./agentPersonalities.js", () => ({
  AgentPersonalityManager: vi.fn().mockImplementation(() => ({
    getPersonality: vi
      .fn()
      .mockReturnValue({ id: "test-agent", name: "Test Agent" }),
  })),
}));
...
describe("DevBotsManager Retry Functionality", () => {
  ...
  describe("retryTask", () => {
    ...
    it("should return error for non-existent task", async () => {
      const result = await manager.retryTask(
        "non-existent-task",
        "Manual retry",
      );
  ...
  describe("cancelRetry", () => {
    it("should return error for manual retry system", () => {
      ...
      expect(result.message).toBe(
        "Manual retry cannot be cancelled once started",
      );
    });
