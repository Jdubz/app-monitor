...
describe("LogWatcher", () => {
  ...
  describe("File Watching", () => {
    ...
    it("should handle file changes", () => {
      ...
      mockFs.watch.mockImplementation(
        (filepath: string, callback: WatchCallback) => {
      ...
      mockFs.readFileSync.mockReturnValue(
        '{"severity":"INFO","timestamp":"2025-01-26T10:00:00.000Z","environment":"development","service":"dev-monitor-backend","message":"Test"}',
      );
  ...
  describe("Log Format Validation", () => {
    it("should detect JSON log format with high confidence", () => {
      ...
      const jsonLogs = Array(10)
        .fill('{"severity":"INFO","message":"Test"}')
        .join("\n");
      mockFs.readFileSync.mockReturnValue(jsonLogs);
      mockFs.existsSync.mockReturnValue(true);

      // When: Format is validated
      const validation = logWatcher["validateLogFormat"](
    ...
    it("should detect plain text log format", () => {
      ...
      const plainLogs = Array(10)
        .fill("2025-01-26 10:00:00 [INFO] Test message")
        .join("\n");
  ...
  describe("Plain Text Conversion", () => {
    it("should convert plain text to structured format", () => {
      ...
      const plainLog =
        "2025-01-26 10:00:00 [INFO] Backend service started on port 5000";

      // When: Log is converted
      const structured = logWatcher["convertPlainTextToStructured"](
        plainLog,
        "backend",
      );
  ...
  describe("Real-time Streaming", () => {
    ...
    it("should handle format errors gracefully", () => {
      ...
      logWatcher["broadcastFormatError"](
        "backend",
        "/test/logs/backend.log",
        invalidLine,
      );
