...
export class ClaudeLogParser {
  ...
  async parseProjectUsage(
    ...
    for (const entry of allEntries) {
      ...
      summary.modelBreakdown[entry.model].inputTokens +=
        entry.usage.totalInputTokens;
      summary.modelBreakdown[entry.model].outputTokens +=
        entry.usage.totalOutputTokens;
      summary.modelBreakdown[entry.model].requestCount += 1;

      // Calculate cost
      const pricing = CLAUDE_PRICING[entry.model] || CLAUDE_PRICING.default;
      const inputCost =
        (entry.usage.totalInputTokens / 1_000_000) * pricing.input;
      const outputCost =
        (entry.usage.totalOutputTokens / 1_000_000) * pricing.output;
