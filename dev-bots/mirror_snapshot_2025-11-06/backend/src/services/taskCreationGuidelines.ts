...
export class TaskCreationGuidelinesManager {
    ...
    this.guidelines.set("implementation", {
      ...
      validationRules: [
        ...
        },
        {
          ...
          rule: "minLength:1",
          message:
            "Must reference at least one architecture document or pattern",
          severity: "error",
        },
      ],
      examples: [
        {
          ...
          example: {
            title: "User Authentication API Endpoint",
            description:
              "Implement REST API endpoint for user authentication with JWT tokens, including login, logout, and token refresh functionality.",
            acceptanceCriteria: [
      ...
      examples: [
        {
          ...
          description:
            "Comprehensive security review of the authentication module for vulnerabilities and best practices",
          example: {
            title: "Security Review of Authentication Module",
            description:
              "Conduct thorough security review of authentication module including password handling, JWT implementation, and session management.",
            acceptanceCriteria: [
      ...
      examples: [
        {
          ...
          description:
            "Create comprehensive unit tests for the user service including all CRUD operations and edge cases",
          example: {
            title: "Unit Tests for User Service",
            description:
              "Implement comprehensive unit tests for user service covering all methods, error scenarios, and edge cases.",
            acceptanceCriteria: [
  ...
  private initializeValidationRules(): void {
    this.validationRules = [
      ...
      },
      {
        ...
        rule: "max:40",
        message:
          "Estimated hours should not exceed 40 (break into smaller tasks)",
        severity: "warning",
      },
    ];
  }
  ...
  public validateTaskData(
    taskData: Partial<EnhancedTaskData>,
    taskType: string,
  ): {
    isValid: boolean;
    errors: string[];
    ...
    switch (rule.rule) {
      ...
      case "singleRepository":
        return typeof value === "string" && value.includes(",")
          ...
          : null;
      case "validProject":
        return typeof value === "string" && !this.validProjects.includes(value)
          ? rule.severity
          : null;
      case "validAgent":
        return typeof value === "string" && !this.validAgents.includes(value)
          ? rule.severity
          : null;
      default:
        return null;
  ...
  private generateSuggestions(
    taskData: Partial<EnhancedTaskData>,
    guidelines: TaskCreationGuidelines,
  ): string[] {
    const suggestions: string[] = [];
    ...
    if (!taskData.parentInitiative) {
      suggestions.push(
        "Consider linking to parent initiative or long-term goal",
      );
    }
    if (!taskData.assumptions) {
      suggestions.push(
        "Consider documenting assumptions to avoid misunderstandings",
      );
    }
    if (!taskData.risks) {
      suggestions.push(
        "Consider identifying potential risks and mitigation strategies",
      );
    }
    ...
    if (guidelines.id === "implementation") {
      if (!taskData.testingRequirements) {
        suggestions.push(
          "Consider specifying testing requirements for implementation",
        );
      }
