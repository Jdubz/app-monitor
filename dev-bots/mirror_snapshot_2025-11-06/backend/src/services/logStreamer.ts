...
export class LogStreamer {
  ...
  constructor(
    ...
    (processManager as ProcessManager & { logWatcher: LogWatcher }).logWatcher =
      this.logWatcher;
  ...
  private handleConnection(socket: Socket): void {
    ...
    socket.on("subscribe_logs", (serviceName: string) => {
      ...
      if (serviceName !== "all") {
        ...
        const logLines = logs.map((entry, index) =>
          this.convertStructuredLogToDevMonitorLog(entry, index),
        );
    ...
    socket.on(
      "get_history",
      (data: { serviceName: string; lines?: number }) => {
    ...
    socket.on("get_service_status", (serviceName: string) => {
      try {
        ...
        socket.emit("error", {
          message: `Failed to get status for ${serviceName}`,
        });
      }
    });
    ...
    socket.on(
      ...
      async (data: {
        ...
        try {
          ...
          socket.emit("error", {
            message: `Failed to subscribe to cloud logs: ${errorMessage}`,
          });
        }
      },
    );
    ...
    socket.on(
      "unsubscribe_cloud_logs",
      (data: { environment: string; service: string }) => {
        logger.info({
          category: "process",
      ...
      async (data: {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          socket.emit("error", {
            message: `Failed to refresh cloud logs: ${errorMessage}`,
          });
        }
      },
    );
    ...
    socket.on(
      "get_file_log_history",
      (data: { service: string; lines?: number }) => {
   ...
   */
  private convertStructuredLogToDevMonitorLog(
   ...
   */
  private broadcastStatusChange(data: {
    serviceName: string;
    status: string;
    error?: string;
  }): void {
