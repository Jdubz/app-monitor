...
export class DockerManager {
  ...
  private static readonly REQUIRED_IMAGES = [DockerManager.CLAUDE_WORKER_IMAGE];
  ...
  async validateDockerEnvironment(): Promise<DockerValidationResult> {
    ...
    try {
      ...
      for (const imageName of DockerManager.REQUIRED_IMAGES) {
        ...
        if (!imageInfo.exists) {
          result.warnings.push(
            `Required image '${imageName}' not found - will attempt to pull`,
          );
        } else {
          logger.info({
            category: "process",
      ...
      result.errors.push(
        `Docker validation failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      result.isValid = false;
      logger.error({
        category: "process",
   ...
   */
  async ensureRequiredImages(): Promise<{
    success: boolean;
    errors: string[];
  }> {
    const errors: string[] = [];
  ...
  async createContainer(
    ...
    if (pullImageIfMissing && imageName) {
      ...
      if (!imageInfo.exists) {
        ...
        if (!pulled) {
          throw new Error(
            `Failed to pull image ${imageName} required for container creation.`,
          );
        }
      }
    }
