...
export class CloudLogging {
      ...
      if (fs.existsSync(config.gcpKeyFile)) {
        ...
        try {
          ...
          logger.info({
            ...
            message:
              "CloudLogging initialized with Application Default Credentials",
          });
          this.initialized = true;
        } catch (adcError) {
          logger.warn({
  ...
  private buildFilter(
    ...
    environment: typeof environments.staging,
  ): string {
    ...
    if (query.service && query.service !== "all-functions") {
      const serviceConfig = environment.services.find(
        (s) => s.name === query.service,
      );
  ...
  private parseLogEntry(
    ...
    if (typeof data === "string") {
      ...
      if ("message" in data && typeof data.message === "string") {
        ...
        "textPayload" in data &&
        typeof data.textPayload === "string"
      ) {
        ...
        typeof data.protoPayload === "object" &&
        data.protoPayload !== null
      ) {
        // Handle protobuf audit logs - extract meaningful information
        message = this.parseProtoPayload(
          data.protoPayload as Record<string, unknown>,
          metadata,
        );
      } else {
    ...
    const entryTimestamp = (entry as unknown as { timestamp?: unknown })
      .timestamp;
    const timestamp = entryTimestamp
      ? typeof entryTimestamp === "object" &&
        entryTimestamp !== null &&
        "seconds" in entryTimestamp
    ...
    const trace =
      typeof metadata.trace === "string" ? metadata.trace : undefined;
    const spanId =
      typeof metadata.spanId === "string" ? metadata.spanId : undefined;

    // Extract resource info
    const resource =
      typeof metadata.resource === "object" && metadata.resource !== null
    ...
    return {
      ...
      metadata: {
        ...
        severity,
        insertId:
          typeof metadata.insertId === "string" ? metadata.insertId : undefined,
        logName:
          typeof metadata.logName === "string" ? metadata.logName : undefined,
      },
      raw: entry,
    };
  }
  ...
  private parseProtoPayload(
    ...
    try {
      ...
      const methodName =
        typeof protoPayload.methodName === "string"
          ? protoPayload.methodName
          : "Unknown method";
      const serviceName =
        typeof protoPayload.serviceName === "string"
          ? protoPayload.serviceName
          : "Unknown service";
      const resourceName =
        typeof protoPayload.resourceName === "string"
          ? protoPayload.resourceName
          : "";
      const requestMetadata =
        typeof protoPayload.requestMetadata === "object" &&
        protoPayload.requestMetadata !== null
          ? (protoPayload.requestMetadata as Record<string, unknown>)
          : ({} as Record<string, unknown>);
      const callerIp =
        typeof requestMetadata.callerIp === "string"
          ? requestMetadata.callerIp
          : "Unknown IP";

      // For Cloud Build/Functions deployment audit logs
      if (
        serviceName.includes("cloudfunctions") ||
        serviceName.includes("cloudbuild")
      ) {
        const status =
          typeof protoPayload.status === "object" &&
  ...
  private mapSeverityToLevel(
    ...
    if (
      severityUpper.includes("ERROR") ||
      severityUpper.includes("CRITICAL") ||
      severityUpper.includes("ALERT") ||
      severityUpper.includes("EMERGENCY")
    ) {
      return "ERROR";
    }
  ...
  private enforceRateLimit(environment: string): void {
    ...
    if (timeSinceLastRequest < this.minRequestInterval) {
      ...
      throw new Error(
        `Rate limit: Please wait ${waitTime}ms before making another request to ${environment}`,
      );
    }
