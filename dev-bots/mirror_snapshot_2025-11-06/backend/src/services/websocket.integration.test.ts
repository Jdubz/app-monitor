...
import {
  TestSocketIOServer,
  TestClientSocket,
} from "../../tests/helpers/fake-socket-server.js";
...
describe("LogStreamer socket integration (in-process)", () => {
  ...
  beforeEach(() => {
    ...
    processManager = {
      ...
      getAllStatuses: vi
        .fn()
        .mockReturnValue([{ name: "job-finder-backend", status: "running" }]),
      getServiceStatus: vi
        .fn()
        .mockReturnValue({ name: "job-finder-backend", status: "running" }),
      startService: vi.fn(),
  ...
  it("sends initial status payloads when a client connects", () => {
    ...
    expect(received[0]).toEqual([
      { name: "job-finder-backend", status: "running" },
    ]);
  });
  ...
  it("returns log history when requested by the client", async () => {
    ...
    (logStreamer as any).logWatcher.getRecentLogs = vi
      .fn()
      .mockReturnValue(mockLogs);

    io.connect(client);
    await client.clientEmit("get_history", {
      serviceName: "backend",
      lines: 5,
    });
  ...
  it("handles cloud logging failures gracefully and notifies client", async () => {
    ...
    io.connect(client);
    await client.clientEmit("refresh_cloud_logs", {
      environment: "staging",
      service: "api",
    });
