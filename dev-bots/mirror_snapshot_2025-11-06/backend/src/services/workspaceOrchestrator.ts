...
export class WorkspaceOrchestrator extends EventEmitter {
  ...
  constructor(config: WorkspaceOrchestratorConfig = {}) {
    ...
    const devBotsRoot =
      config.devBotsRoot || path.resolve(process.cwd(), "../dev-bots");
    this.mirrorPath = config.mirrorPath || path.join(devBotsRoot, "mirror");
    this.workspaceRoot =
      config.workspaceRoot || path.join(devBotsRoot, "workspaces");
    this.artifactsRoot =
      config.artifactsRoot || path.join(devBotsRoot, "artifacts");
  ...
  async sealWorkspace(
    ...
    const sanitizedTitle = options.taskTitle
      .replace(/[\r\n]+/g, " ")
      .slice(0, 120);
  ...
  private getRemoteUrl(): string {
    if (!this.remoteUrl) {
      this.remoteUrl = execFileSync("git", ["remote", "get-url", "origin"], {
        cwd: this.repoRoot,
      })
        .toString()
        .trim();
  ...
  public createPatchArtifact(context: WorkspaceContext): string {
    ...
    try {
      const patch = execFileSync(
        "git",
        ["format-patch", `origin/${this.branch}`, "--stdout"],
        {
          cwd: context.hostPath,
        },
      );
