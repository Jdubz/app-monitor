...
router.post("/services/:name/docker/start-container", async (req, res) => {
  ...
  if (name !== "python-worker") {
    return res
      .status(400)
      .json({ error: "Docker operations only supported for python-worker" });
  }
...
export async function getDockerContainerInfo(containerName: string): Promise<{
  ...
  try {
    ...
    const startedAt = startTimeStr.trim()
      ? new Date(startTimeStr.trim()).getTime()
      : null;
    ...
    console.error(
      `[DOCKER] Failed to get container info for ${containerName}:`,
      error,
    );
    return {
      running: false,
    ...
    <div className="docker-status">
      ...
      {service.dockerContainer.workerStatus && (
        <div className="status-row">
          ...
          <span
            className={`badge badge-${service.dockerContainer.workerStatus}`}
          >
            {service.dockerContainer.workerStatus}
          </span>
        </div>
