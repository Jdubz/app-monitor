...
describe("API Service", () => {
  ...
  describe("Script Management", () => {
    ...
    it("should fetch a specific execution", async () => {
      vi.mocked(mockAxiosInstance.get).mockResolvedValue({
        data: mockScriptExecution,
      } as any);

      const result = await apiModule.getExecution("exec-1");

      expect(mockAxiosInstance.get).toHaveBeenCalledWith(
        "/scripts/executions/exec-1",
        undefined,
      );
    ...
    it("should kill a script execution", async () => {
      ...
      expect(mockAxiosInstance.post).toHaveBeenCalledWith(
        "/scripts/executions/exec-1/kill",
        undefined,
        undefined,
      );
      expect(result).toEqual(response);
