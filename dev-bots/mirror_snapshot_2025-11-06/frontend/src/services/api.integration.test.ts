...
describe("API Service Integration Tests", () => {
  ...
  describe("Service Management Integration", () => {
    ...
    it("should handle network errors", async () => {
      ...
      await expect(apiModule.startService("test-service")).rejects.toThrow(
        "Network error",
      );
    });
  });
  ...
  describe("Script Management Integration", () => {
    it("should handle complete script execution lifecycle", async () => {
      ...
      vi.mocked(mockApiClient.get).mockResolvedValueOnce({
        count: 1,
        executions: [mockExecutionSummary],
      });
  ...
  describe("Error Handling Integration", () => {
    it("should handle different types of API errors", async () => {
      ...
      await expect(apiModule.getServiceStatus("nonexistent")).rejects.toThrow(
        "Not Found",
      );
