...
export const DevBotsPanel: React.FC<DevBotsPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "tasks" | "metrics" | "cleanup" | "agents" | "templates"
  >("overview");
  ...
  const triggerEmergencyRecovery = async () => {
    try {
      ...
      setError(
        err.response?.data?.error || "Failed to trigger emergency recovery",
      );
    }
  };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      socket.off(
        "claude:coordinatorHealthChange",
        handleCoordinatorHealthChange,
      );
    };
  }, [socket]);
      ...
      <div className={styles["panel-content"]}>
            ...
            <div className={styles["status-section"]}>
              ...
              <div className={styles["status-grid"]}>
                ...
                <div className={styles["status-item"]}>
                  ...
                  <span className={styles["value"]}>
                    {status ? Math.round(status.uptime) : 0}s
                  </span>
                </div>
                <div className={styles["status-item"]}>
                  <span className={styles["label"]}>Workers:</span>
                  <span className={styles["value"]}>
                    {status?.workerCount || 0}/{status?.maxWorkers || 2}
                  </span>
                </div>
                <div className={styles["status-item"]}>
                  <span className={styles["label"]}>Active Types:</span>
                  <span className={styles["value"]}>
                    {status?.activeWorkerTypes?.join(", ") || "None"}
                  </span>
                </div>
                <div className={styles["status-item"]}>
                  <span className={styles["label"]}>Available Types:</span>
                  <span className={styles["value"]}>
                    {status?.availableWorkerTypes?.join(", ") || "None"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className={styles["workers-section"]}>
              ...
              {!status?.workers || Object.keys(status.workers).length === 0 ? (
                <div className={styles["no-workers"]}>
                  <p>
                    No active workers. Workers are created ephemerally for each
                    task and destroyed when complete.
                  </p>
                </div>
              ) : (
                <div className={styles["workers-grid"]}>
                  {Object.entries(status.workers).map(([workerId, worker]) => (
                    <div
                      key={workerId}
                      className={`${styles["worker-card"]} ${styles[`worker-${worker.status}`]}`}
                    >
                      ...
                      <div className={styles["worker-info"]}>
                        ...
                        <div className={styles["info-item"]}>
                          ...
                          <div className={styles["info-value"]}>
                            {worker.personality?.name || "Unknown"}
                          </div>
                        </div>
                        <div className={styles["info-item"]}>
              ...
              <div className={styles["task-form"]}>
                ...
                <div className={styles["form-row"]}>
                  <select
                    ...
                    onChange={(e) =>
                      setNewTask({ ...newTask, assignedAgent: e.target.value })
                    }
                    className={styles["form-input"]}
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setNewTask({ ...newTask, documentation: e.target.value })
                  }
        ...
        {activeTab === "tasks" && (
          <div className={styles["tasks-section"]}>
            ...
            {status?.tasks.pending && status.tasks.pending.length > 0 && (
              <div className={styles["task-group"]}>
                ...
                {status.tasks.pending.map((task) => (
                  <div
                    key={task.id}
                    className={`${styles["task-item"]} ${styles.pending}`}
                  >
                    ...
                    <div className={styles["task-meta"]}>
                      ...
                      {task.scope && (
                        <span className={styles["scope-indicator"]}>
                          🔒 Scoped
                        </span>
                      )}
                    </div>
            ...
            {status?.tasks.active && status.tasks.active.length > 0 && (
              <div className={styles["task-group"]}>
                ...
                {status.tasks.active.map((task) => (
                  <div
                    key={task.id}
                    className={`${styles["task-item"]} ${styles.active}`}
                  >
                    <div className={styles["task-header"]}>
                      <span className={styles["task-id"]}>{task.id}</span>
                      <span className={styles["task-worker"]}>
                        {task.assignedWorker}
                      </span>
                      {task.assignedAgent && (
                    ...
                    <div className={styles["task-meta"]}>
                      Started:{" "}
                      {formatTimestamp(task.assignedAt || task.createdAt)}
                      {task.project && (
                        <span className={styles["project-indicator"]}>
                          📁 {task.project}
                        </span>
                      )}
                      {task.files && task.files.length > 0 && (
                        <span className={styles["files-indicator"]}>
                          📄 {task.files.length} files
                        </span>
                      )}
                      {task.isEmergency && (
                        <span className={styles["emergency-indicator"]}>
                          🚨 Emergency
                        </span>
                      )}
                    </div>
            ...
            {status?.tasks.completed && status.tasks.completed.length > 0 && (
              <div className={styles["task-group"]}>
                ...
                {status.tasks.completed.slice(-10).map((task) => (
                  <div
                    key={task.id}
                    className={`${styles["task-item"]} ${styles.completed} ${getActivityClass(task) ? styles[getActivityClass(task)] : ""}`}
                  >
                    <div className={styles["task-header"]}>
                      <span className={styles["task-id"]}>{task.id}</span>
                      <span
                    ...
                    <div className={styles["task-meta"]}>
                      ...
                      {task.scopeViolations &&
                        task.scopeViolations.length > 0 && (
            ...
            {!status?.tasks.pending?.length &&
              !status?.tasks.active?.length &&
              !status?.tasks.completed?.length && (
                ...
                {scopeViolations.slice(-5).map((violation, index) => (
                  <div key={index} className={styles["violation-item"]}>
                    <div className={styles["violation-header"]}>
                      <span className={styles["violation-task"]}>
                        {violation.taskId}
                      </span>
                      <span className={styles["violation-count"]}>
                    ...
                    <div className={styles["violation-details"]}>
                      {violation.violations.map((v, i) => (
                        <span
                          key={i}
                          className={`${styles["violation-type"]} ${styles[v.severity.toLowerCase()]}`}
                        >
                          {v.type} ({v.severity})
                        </span>
                      ))}
                  ...
                  status.tasks.completed
                    ...
                    .map((task) => {
                      ...
                      return (
                        <div
                          key={task.id}
                          className={`${styles["activity-item"]} ${getActivityClass(task) ? styles[getActivityClass(task)] : ""}`}
                        >
              ...
              <div className={styles["cleanup-status"]}>
                ...
                <div className={styles["cleanup-info"]}>
                  <div className={styles["cleanup-item"]}>
                    ...
                    <span className={styles["value"]}>
                      {cleanupStatus.totalCleanupTasks}
                    </span>
                  </div>
                  <div className={styles["cleanup-item"]}>
                    <span className={styles["label"]}>Due Schedules:</span>
                    <span className={styles["value"]}>
                      {cleanupStatus.schedules.length}
                    </span>
                  </div>
                </div>
                ...
                {cleanupStatus.recentTasks.length > 0 && (
                  <div className={styles["recent-cleanup-tasks"]}>
                    ...
                    {cleanupStatus.recentTasks.slice(-5).map((task) => (
                      <div
                        key={task.id}
                        className={styles["cleanup-task-item"]}
                      >
                        <div className={styles["task-header"]}>
                          <span className={styles["task-id"]}>{task.id}</span>
                  ...
                  <div className={styles["agent-specialties"]}>
                    ...
                    <div className={styles["specialty-tags"]}>
                      {agent.specialties.map((specialty) => (
                        <span
                          key={specialty}
                          className={styles["specialty-tag"]}
                        >
                          {specialty}
                        </span>
                      ))}
