...
const CloudPanelContainer: React.FC<CloudPanelContainerProps> = ({
  ...
  const defaultEnvironment = useMemo(() => {
    return deployedEnvironments.staging
      ? "staging"
      : Object.keys(deployedEnvironments)[0] || "staging";
  ...
  useEffect(() => {
    if (panels.length > 0) {
      ...
      const regularPanels = panels.map(
        ({ environment: _environment, service: _service, ...panel }) => panel,
      );
  ...
  useEffect(() => {
    const fetchAllServices = async () => {
      try {
        ...
        const allServicesPromises = Object.keys(deployedEnvironments).map(
          async (env) => {
          ...
          return (
            <CloudPanelWrapper
              ...
              onSourceChange={(source: LogSource) =>
                handleSourceChange(panel.id, source)
              }
              onMetadataToggle={() =>
...
const CloudPanelWrapper: React.FC<CloudPanelWrapperProps> = ({
  ...
  const panelServices = services.filter(
    (svc) => (svc as any).environment === panel.environment,
  );
