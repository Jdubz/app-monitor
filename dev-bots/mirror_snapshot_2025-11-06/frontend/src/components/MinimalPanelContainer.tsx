...
const MinimalPanelContainer: React.FC = () => {
  const { getLogsForService, isConnected, subscribeToService } =
    useLogContext();
  ...
  useEffect(() => {
    const fetchSources = async () => {
      try {
        ...
        const serviceNames = logSources.map(
          (source) => source.id as LocalService,
        );
    ...
    <div style={containerStyle}>
      ...
      <div style={panelGridStyle}>
        {panels.map((panel) => {
          ...
          const hasError =
            panel.source !== null && !availableSources.includes(panel.source);
