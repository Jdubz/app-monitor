...
export const useServices = () => {
        ...
        setServices((prev) =>
          prev.map((service) =>
            service.name === data.serviceName
              ? {
                  ...service,
                  status: data.status as ProcessInfo["status"],
                  error: data.error,
                }
              : service,
          ),
  ...
  const handleStart = useCallback(async (serviceName: string) => {
    ...
    setServices((prev) =>
      prev.map((s) =>
        s.name === serviceName
          ? { ...s, status: "starting" as const, error: undefined }
          : s,
      ),
    );
  ...
  const handleStop = useCallback(
    async (serviceName: string, graceful: boolean = true) => {
      // Optimistic update
      setServices((prev) =>
        prev.map((s) =>
          s.name === serviceName
            ? { ...s, status: "stopping" as const, error: undefined }
            : s,
        ),
      );
