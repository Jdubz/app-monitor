...
/**
 * Safe Test Runner - app-monitor-frontend
 *
...
class SafeTestRunner {
  ...
  acquireLock() {
    ...
    const lockData = {
      ...
      repository: "app-monitor-frontend",
      testSuite: "unit",
    };
  ...
  startResourceMonitoring() {
    const monitorInterval = setInterval(() => {
      ...
      if (executionTime > MAX_EXECUTION_TIME) {
        console.error(
          "\n⚠️  CRITICAL: Test execution exceeded " +
      ...
      if (executionTime % 30000 < 1000) {
        console.log(
          "[Monitor] Memory: " +
            memUsageMB.toFixed(1) +
            "MB | Time: " +
            (executionTime / 1000).toFixed(1) +
            "s",
        );
      }
    }, 1000);
  ...
  terminateTests() {
    ...
    try {
      if (process.platform === "win32") {
        require("child_process").execSync("taskkill /F /IM vitest* /T", {
          stdio: "inherit",
        });
      } else {
        require("child_process").execSync("pkill -9 -f vitest", {
          stdio: "inherit",
        });
      }
    } catch (error) {
  ...
  async run() {
    console.log("\n🛡️  Safe Test Runner - app-monitor-frontend\n");
