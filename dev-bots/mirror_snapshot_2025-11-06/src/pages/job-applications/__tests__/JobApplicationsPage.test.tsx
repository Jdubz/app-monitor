...
vi.mock("../components/JobDetailsDialog", () => ({
  JobDetailsDialog: ({ match, open, onOpenChange, onGenerateResume }: any) => (
    ...
    >
      ...
      <button
        onClick={() => onGenerateResume?.(match)}
        data-testid="generate-resume"
      >
        Generate Resume
      </button>
    </div>
...
describe("JobApplicationsPage", () => {
  ...
  describe("rendering", () => {
    it("should render job applications page with header", async () => {
      ...
      expect(
        screen.getByText("AI-matched opportunities ranked by relevance"),
      ).toBeInTheDocument();
    });
  ...
  describe("job matches display", () => {
    it("should display job match cards when matches are available", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId("job-match-card-match-1"),
        ).toBeInTheDocument();
        expect(
          screen.getByTestId("job-match-card-match-2"),
        ).toBeInTheDocument();
        expect(
          screen.getByTestId("job-match-card-match-3"),
        ).toBeInTheDocument();
      });
  ...
  describe("search and filtering", () => {
    it("should filter job matches by search query", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.queryByTestId("job-match-card-match-2"),
        ).not.toBeInTheDocument();
        expect(
          screen.queryByTestId("job-match-card-match-3"),
        ).not.toBeInTheDocument();
      });
    });
    ...
    it("should show no results message when filters return no matches", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("No matches found for your current filters"),
        ).toBeInTheDocument();
  ...
  describe("job details dialog", () => {
    ...
    it("should close job details dialog when close button is clicked", async () => {
      ...
      expect(
        screen.queryByTestId("job-details-dialog"),
      ).not.toBeInTheDocument();
    });
  ...
  describe("error handling", () => {
    it("should show error message when job matches fail to load", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            "Failed to load job matches. Please refresh the page.",
          ),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("real-time updates", () => {
    ...
    it("should unsubscribe from job matches on unmount", () => {
      ...
      vi.mocked(jobMatchesClient.subscribeToMatches).mockReturnValue(
        unsubscribe,
      );
    ...
    it("should update matches when new data is received", async () => {
      ...
      expect(
        screen.queryByTestId("job-match-card-match-1"),
      ).not.toBeInTheDocument();
  ...
  describe("accessibility", () => {
    it("should have proper form labels and ARIA attributes", async () => {
      ...
      expect(
        screen.getByPlaceholderText("Search by company or job title..."),
      ).toBeInTheDocument();
    });
  ...
  describe("edge cases", () => {
    ...
    it("should handle job matches with missing optional fields", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId("job-match-card-match-incomplete"),
        ).toBeInTheDocument();
      });
