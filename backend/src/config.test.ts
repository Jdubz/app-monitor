  ...
  describe('Scripts Configuration', () => {
    ...
    it('should have valid script structure for all scripts', () => {
      Object.values(scripts).forEach(script => {
        ...
        expect(['build', 'test', 'quality', 'database', 'deployment', 'utility']).toContain(script.category)
    ...
    it('should categorize scripts correctly', () => {
      const testScripts = Object.values(scripts).filter(s => s.category === 'test')
      const buildScripts = Object.values(scripts).filter(s => s.category === 'build')
      const databaseScripts = Object.values(scripts).filter(s => s.category === 'database')
    ...
    it('should have danger levels for dangerous scripts', () => {
      const dangerousScripts = Object.values(scripts).filter(s =>
        s.id.includes('clear') || s.id.includes('production') || s.id.includes('copy')
      )
    ...
    it('should have frontend scripts', () => {
      const feScripts = Object.values(scripts).filter(s => s.id.startsWith('fe-'))
    ...
    it('should have backend scripts', () => {
      const beScripts = Object.values(scripts).filter(s => s.id.startsWith('be-'))
    ...
    it('should have worker scripts', () => {
      const workerScripts = Object.values(scripts).filter(s => s.id.startsWith('worker-'))
