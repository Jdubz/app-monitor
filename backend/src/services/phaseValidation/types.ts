 ...
 */
export class PhaseValidatorRegistry {
  private validators: Map<number, PhaseValidator> = new Map();

  /**
   * Register a validator for a specific phase.
   */
  register(validator: PhaseValidator): void {
    if (this.validators.has(validator.phaseIndex)) {
      throw new Error(`Validator for phase ${validator.phaseIndex} already registered`);
    }
    this.validators.set(validator.phaseIndex, validator);
  }

  /**
   * Get validator for a phase.
  ...
  getValidator(phaseIndex: number): PhaseValidator {
    ...
    if (!validator) {
      throw new Error(`No validator registered for phase ${phaseIndex}`);
    }
    return validator;
  }
   ...
   * Delegates to the appropriate validator.
   */
  async validate(phaseIndex: number, task: Task, artifacts: PhaseArtifacts): Promise<ValidationResult> {
    const validator = this.getValidator(phaseIndex);
    return validator.validate(task, artifacts);
  }
}
