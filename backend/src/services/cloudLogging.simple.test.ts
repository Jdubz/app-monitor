...
describe('CloudLogging Simple Tests', () => {
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error for unknown environment', async () => {
      ...
      // When: Getting logs
      // Then: Error is thrown
      await expect(cloudLogging.getLogs(query)).rejects.toThrow('Environment "unknown" not found');
    });
    ...
    it('should handle Google Cloud API errors', async () => {
      ...
      // When: Getting logs
      // Then: Error is thrown
      await expect(cloudLogging.getLogs(query)).rejects.toThrow('Failed to fetch cloud logs: API Error');
    });
  });
