...
describe('ClaudeWorkersManager Worker Limit Enforcement', () => {
  let claudeWorkersManager: ClaudeWorkersManager;
  ...
  beforeEach(() => {
    ...
    claudeWorkersManager = new ClaudeWorkersManager(mockProcessManager);

    // Mock initializeWorkerLogFile to avoid filesystem operations
    claudeWorkersManager['initializeWorkerLogFile'] = vi.fn().mockResolvedValue(undefined);

    // Mock initial state
    claudeWorkersManager['taskQueue'] = [];
    claudeWorkersManager['activeTasks'] = new Map();
    claudeWorkersManager['completedTasks'] = [];
    claudeWorkersManager['ephemeralWorkers'] = new Map();
  ...
  describe('Worker Limit Enforcement', () => {
    it.skip('should allow first task to be assigned to worker-a (integration test - fix in Phase 2)', async () => {
      ...
      claudeWorkersManager['taskQueue'].push(task as any);
      ...
      claudeWorkersManager['docker'] = mockDocker as any;

      await claudeWorkersManager.assignNextTask();
    ...
    it.skip('should allow second task to be assigned to worker-b (integration test - fix in Phase 2)', async () => {
      ...
      claudeWorkersManager['ephemeralWorkers'].set('worker-a-test-123', {
    ...
    it('should prevent third task assignment when both worker-a and worker-b are active', async () => {
      ...
      claudeWorkersManager['ephemeralWorkers'].set('worker-b-test-456', {
    ...
    it('should respect MAX_CONCURRENT_WORKERS limit', () => {
      expect(claudeWorkersManager.getMaxWorkers()).toBe(2);
    });
    ...
    it('should handle worker cleanup errors gracefully', async () => {
      ...
      await expect(claudeWorkersManager['destroyEphemeralWorker']('worker-a-test-123')).resolves.not.toThrow();
    });
  });
  ...
  describe('Worker Type Assignment Logic', () => {
    it('should correctly identify active worker-a', () => {
      ...
      const activeWorkers = Array.from(claudeWorkersManager['ephemeralWorkers'].values()).filter(
