...
describe('DockerManager', () => {
  ...
  describe('validateDockerEnvironment', () => {
    ...
    it('should detect missing Docker daemon', async () => {
      ...
      expect(result.errors).toContain('Docker daemon is not running or not accessible');
    });
    ...
    it('should detect permission errors', async () => {
      ...
      mockDocker.listImages.mockResolvedValue([]);
      mockDocker.listContainers.mockRejectedValue(new Error('Permission denied'));

      const result = await dockerManager.validateDockerEnvironment();

      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Insufficient permissions to access Docker socket');
    });
  ...
  describe('pullImage', () => {
    it('should pull image successfully', async () => {
      ...
      mockDocker.modem.followProgress.mockImplementation((stream: any, onFinished: any) => {
        onFinished(null, []);
      });
    ...
    it('should report progress', async () => {
      ...
      mockDocker.modem.followProgress.mockImplementation((stream: any, onFinished: any, onProgress: any) => {
