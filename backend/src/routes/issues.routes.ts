...
import type { IssueReportRequest, IssueReportApiResponse } from '@app-monitor/api-contracts';
...
const issueReportLimiter = rateLimit({
  ...
  keyGenerator: (req) => {
    ...
    const body = req.body as IssueReportRequest;
  ...
  handler: (req, res) => {
    logger.warn({
      ...
      details: {
        sessionId: (req.body as IssueReportRequest)?.sessionId,
        ip: req.ip,
      },
    });
    res.status(429).json({
 ...
 */
const issueReportRoute = defineRoute({
  method: 'post',
  path: '/',
  summary: 'Submit Issue Report',
  description: 'Receive an issue report from the frontend and trigger immediate triage.',
  tags: ['issues'],
  request: {
    body: {} as IssueReportRequest
  },
  response: {
    body: {} as IssueReportApiResponse
  },
  handler: async (req: Request, res: Response) => {
    try {
      const report = req.body as IssueReportRequest;
      ...
      const responsePayload: IssueReportApiResponse = {
        success: true,
        data: {
          issueId: issue.id,
...
router[issueReportRoute.method](
  issueReportRoute.path,
  express.json({ limit: '10mb' }),
  issueReportLimiter,
  issueReportRoute.handler
);
