...
async function findExistingProcesses() {
  try {
    ...
    const { stdout } = await execAsync('ps aux | grep -E "nodemon.*dev-monitor|tsx.*dev-monitor" | grep -v grep');
...
async function main() {
  ...
  if (existingProcesses.length > 0) {
    ...
    console.error('   1. Kill all dev-monitor processes: pkill -f "nodemon.*dev-monitor"');
  ...
  if (portProcesses.length > 0) {
    ...
    for (const pid of portProcesses) {
      try {
        const { stdout } = await execAsync(`ps -p ${pid} -o pid,cmd --no-headers`);
    ...
    console.error('');
    console.error('💡 Solutions:');
    console.error('   1. Kill processes using port 5000: lsof -ti:5000 | xargs kill');
