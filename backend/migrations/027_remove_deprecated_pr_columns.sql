-- Migration 027: Remove Deprecated PR Columns
-- 
-- Removes duplicate PR workflow columns that have been migrated to
-- the dedicated pull_requests table (migration 007).
-- 
-- Only pr_number is retained as a foreign key reference.
--
-- Columns being removed:
-- - pr_url (DEPRECATED - use pull_requests.url)
-- - pr_branch (DEPRECATED - use pull_requests.head_branch)
-- - pr_status (DEPRECATED - use pull_requests.state)
-- - pr_checks_status (DEPRECATED - use pull_requests.check_suite_status)
-- - pr_review_status (DEPRECATED - use pull_requests.review_decision)
-- - pr_created_at (DEPRECATED - use pull_requests.created_at)
-- - pr_merged_at (DEPRECATED - use pull_requests.merged_at)
...
CREATE TABLE tasks_new (
  ...
  metadata TEXT,
  context_json TEXT,
  -- Migration 005: RETAINED pr_number as foreign key to pull_requests table
  pr_number INTEGER,
...
SELECT 
  ...
  -- Migration 005 (ONLY pr_number retained)
  pr_number,
  -- Migration 011
...
-- Migration complete: 7 deprecated PR columns removed, pr_number retained
