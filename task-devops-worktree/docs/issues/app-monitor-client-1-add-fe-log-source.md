# APP-MONITOR-CLIENT-1 — Add Frontend Client as Log Source

## Issue Metadata

```yaml
Title: APP-MONITOR-CLIENT-1 — Add Frontend Client as Log Source
...
**ENHANCEMENT**: Add the job-finder-FE client as a log source in the app-monitor, including implementing the log streaming from the client in the job-finder-FE repository. For staging and production, the job-finder-FE client should log to Google Cloud Logging.
...
The app-monitor provides centralized monitoring for all development services, including backend services, worker processes, and now frontend client applications.
...
- Frontend client logs are captured and streamed to app-monitor
...
CREATE:
- app-monitor/backend/src/services/frontendLogCapture.ts - Frontend log capture service
- app-monitor/backend/src/services/cloudLogging.ts - Google Cloud Logging integration
- app-monitor/backend/src/types/frontendLogTypes.ts - Frontend log type definitions
- app-monitor/backend/src/config/cloudLogging.ts - Cloud logging configuration

MODIFY:
- app-monitor/backend/src/services/logCapture.ts - Add frontend log handling
- app-monitor/backend/src/routes/logs.ts - Add frontend log endpoints
- app-monitor/frontend/src/components/LogViewer.tsx - Add frontend log display
...
**Dependencies**: @google-cloud/logging, ws, existing app-monitor infrastructure
...
3. **Implement Log Streaming**
   - Create WebSocket connection between frontend and app-monitor
...
4. **Add Google Cloud Integration**
   ...
   - Implement log forwarding from app-monitor to Google Cloud
...
- Depends on: Existing app-monitor infrastructure, job-finder-FE application
...
- Test frontend log streaming to app-monitor
- Test Google Cloud Logging integration
- Test log filtering and display

**Manual Testing Checklist**

- [ ] Frontend logs are captured and streamed to app-monitor
...
```bash
# Start app-monitor with frontend log capture
npm run dev
...
client-side logging utilities and app-monitor integration.
